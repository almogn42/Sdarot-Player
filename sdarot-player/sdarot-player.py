...
def Argument_Parser(arguments):
    ...
    if len(arguments) > 1:
        ...
        if arguments[1].lower() == "--config":
            ...
            User_Name = input("Please Enter your User Name For Sdarot site (dont worry it is used only for looging in at the start and only for viewing episodes :-) ): \n ")
            User_Password = input("Please Enter your Password For Sdarot site (dont worry it is used only for looging in at the start and only for viewing episodes :-) ): \n ")
            #checking for Driver
            ...
            if Use_MPV.lower() == "true":
                ...
                Conf_File.write(f"UserName = '{User_Name}'\n")
                Conf_File.write(f"Password = '{User_Password}'\n")
                Conf_File.close()
...
def Login_Func(Driver, Sdarot_Link, UserName, Password):
    """ A function for the lodin process usage:Login_Func(Driver, Sdarot_Link, User Name, Password)"""
    ...
    with requests.Session() as Login_Session:
        #matching user-agent and cookies for consistant login
        ...
        for cookie in Driver.get_cookies():
            ...
            #THE Holy POST!!!!!!!!!!!!!!!!!!!
            Login_Session.post(f"{Sdarot_Link}/login",data = Login_Body_Request)
...
#making sure and Variable Defining for username and password
try:
    if Lines[6].split(r"'")[1].strip() == "" :
        input(" You dont have A User Name configured Please rerun the Script with --config to config User Name & Password \n Press any key to quit: ")
        exit()
    else:
        User_Name = Lines[6].split(r"'")[1]
except:
    input(" You dont have A User Name configured Please rerun the Script with --config to config User Name & Password \n Press any key to quit: ")
    exit()

try:
    if Lines[7].split(r"'")[1].strip() == "" :
        input(" You dont have A User Name configured Please rerun the Script with --config to config User Name & Password /n Press any key to quit: ")
        exit()
    else:
        User_Password = Lines[7].split(r"'")[1]
except:
    input(" You dont have A User Name configured Please rerun the Script with --config to config User Name & Password \n Press any key to quit: ")
    exit()
...
#getting if to resume a show or search
while True:
    try:
        Script_Mode = int(input(" Please select Script Option \n [1]. Resume From History \n [2]. Search Show "))
        break
    except:
...
if Script_Mode == 1:
    ...
    Show_ID, Show_Name = ShowSearch(Search_String)
    Season = EpSe_Selector(Show_ID,"Se")
    Episode =EpSe_Selector(Show_ID,"Ep",Season)
...
#there is if statment for the range option that need to repeat the whole sequence per episode in sequence
    ...
    if len(Episode) == 1:
        while True:
            ...
            Url = f"{Sdarot_Link}/watch/{Show_ID}/season/{Season}/episode/{Episode[0]}"
            ...
            if WebDriverStatus == "Closed":
                ...
                if Lines[3].split(r"'")[1] == "False":
                    ...
                    driver.get(Sdarot_Link)
                    #logging in
                    Login_Func(driver, Sdarot_Link, User_Name, User_Password)
            ...
            try:
                ...
                if Lines[0].strip() == "Use MPV = True":
                    ...
                    #updating History_File
                    History_Keep("Write", Show_ID, Show_Name, Season, Episode)


                    #managing logic for after the epishode is played
                    Episode = After_Played_Options(Episode[0], Show_ID, Season)
                    break
    ...
    #for when there is range you need to repeat the episode opening sequendce for each episode in sequence
        ...
        for Ep in range(Episode[0], (int(Episode[1]) + 1)):
            while True:
                ...
                #Making the Complete Url
                Url = f"{Sdarot_Link}/watch/{Show_ID}/season/{Season}/episode/{Ep}"
                #Running selenium driver to open timer for Episode
                    #this is for not restarting the web druver in every loop run
                if WebDriverStatus == "Closed":
                    print("opening browser")
        ...
        Episode = After_Played_Options(Ep, Show_ID, Season)
